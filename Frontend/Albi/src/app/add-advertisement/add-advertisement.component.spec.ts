...
describe('AddAdvertisementComponent', () => {
  ...
  it('#advertisementCreated should be true after creating an advertisement', () => {
    ...
    expect(comp.advertisementCreated).toBe(false, 'when the flat is not created yet');
    comp.createFlat(30000, 3, "Kiváló ház", "t@t.com", "1234545", 2222, "Aggtelek", "Mészkő utca 1.", "underlease");
    //expect(comp.advertisementCreated).toBe(true, 'when the flat is created already');

    expect(comp.createdFlat.price).toMatch('30000', 'after created');
    expect(comp.createdFlat.email).toMatch('t@t.com', 'after created');
  });
